	...
	<reps:panel id="mybody" dock="center">
		<reps:grid id="orglist" items="${list}" form="queryForm" var="table" pagination="${pager}" flagSeq="true">
			<reps:gridrow>
				...
				<reps:gridfield title="数据表名" width="20">${table.name}</reps:gridfield>
				<reps:gridfield title="是否基础表" width="8" align="center">
					<c:if test="${table.isBasic==1}">是</c:if>
				</reps:gridfield>
				<reps:gridfield title="统计方法" width="8" align="center">
					<c:if test="${table.method=='V'}">简单</c:if>
					<c:if test="${table.method=='H'}">复杂</c:if>
				</reps:gridfield>
				<reps:gridfield title="统计优先级" width="8" align="center">${table.priority}</reps:gridfield>
				<reps:gridfield title="操作" width="34">
...
//		}
	...
	function createTemp(){
		...
		htmlItem += '指标名称:</td><td class="fInput" style="width:30%" ><input class="txtInput  required"  maxlength="30" style="width:176px;background-color: rgb(255, 255, 255);" id="itemName'+createItemCount+'"  name="itemName"';
		...
		htmlItem += '字段类型:</td><td class="fInput"><select name="fieldType" id="fieldType'+createItemCount+'" style="width:180px;">';
		...
		htmlItem += '字段长度:</td><td class="fInput"><input class="txtInput number required" maxlength="5" style="width:176px"  name="fieldLength" id="fieldLength'+createItemCount+'" /></td></tr>';
		
		htmlItem += '<tr><td class="fLabel">';
		htmlItem += '表达式类型:</td><td><select class="required" style="width:180px;" name="expressionType" id="expressionType'+createItemCount+'">';
		...
		htmlItem += '<td class="fLabel">';
		htmlItem += '条件依赖:</td><td class="fInput"><input  class="txtInput  valid"  maxlength="200" style="width:176px" id="dependency'+createItemCount+'" name="dependency"';
		htmlItem += '></td></tr>';
	   
		htmlItem += '<tr><td class="fLabel">';
		htmlItem += '表达式:</td><td colspan="3" class="fInput"><textarea class="txtInput valid" name="expression" id="expression'+createItemCount+'" style="width:530px;height:50px;resize:none">';
	...
	function createItem(result, type, isBasic){//isBasic 是否基础表 1:是基础表（则不显示是唯一批次约束项）0：是业务表（则显示是唯一批次约束项）
		...
		htmlItem += '<td class="fLabel">';
		htmlItem += '字段长度:</td><td class="fInput"><input class="txtInput number required" maxlength="5" style="width:176px" value="'+result.fieldLength+'" name="fieldLength" id="fieldLength'+createItemCount+'" /></td></tr>';
